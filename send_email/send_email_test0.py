...
subject="security twitters of "+utc_time.strftime("%Y-%m-%d")
string =subject+"\n\n"
cur = conn.cursor()
...
cur.execute("select * from twitter_info_new where unix_timestamp(time) >=unix_timestamp(%s) and link is not null and focus >30 order by hot_rate desc",(utc_time_start,))
results = cur.fetchall()
i=0
result_list=[]
results_info=[]
for row in results:
    ...
    timestamp_varas=row[18]-datetime.timedelta(hours=8)
    retweeted_status_created_at=row[10]
    if retweeted_status_created_at:
        if retweeted_status_created_at<timestamp_varas-
    ...
    wordlist=jieba.analyse.extract_tags(text1,20)
    predict_list=[]
    predict=0
    if wordlist:
        for item in wordlist:
            cur.execute("select count from keywords_vulnerability where keyword=%s",(item,))
        ...
        if predict_list:
            for item in predict_list:
                predict+=item
    if retweeted_status_id and retweeted_status_user:
...
for i in range(len(results_info)):
    string+=str(i+1)+". "+results_info[i][0]+'Relevancy:'+str(results_info[i][1])+"\n"
print string
