...
def get_content_from_html(href):
    ...
    content=re.subn('\n',' ',content)[0]
    return content,tags
def save_tweet(new_tweet,total):
    ...
    if re.findall('https?://\S+\.\S+\w',content):
        ...
        link=None
    param=(tweet_id,user_name,content,link,tags,created_at,in_reply_to_status_id,quoted_status_id,retweeted_status_id,retweeted_status_user,retweeted_status_created_at,retweet_count,favorite_count)
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("insert ignore into twitter_info(id,screen_name,content,link,tags,time,in_reply_to_status_id,quoted_status_id,retweeted_status_id,retweeted_status_user,retweeted_status_created_at,retweet_count,favorite_count) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",param)
        conn.commit()
        cur.close()
    except Exception as e:
...
def creat_threads(new_tweets,total):
    conn= MySQLdb.connect(
            host='172.18.100.5',
            port = 3306,
            user='jobadmin',
    ...
    cur = conn.cursor()
    cur.execute("select count(*) from twitter_info where screen_name= %s and id between %s and %s",(new_tweets[0].user.screen_name,new_tweets[0].id,new_tweets[-1].id))
    results = cur.fetchone()
    cur.close()
    if results==len(new_tweets):
        return
...
def get_all_tweets(msg):
    ...
    while (len(new_tweets) > 0):
        ...
        creat_threads(new_tweets,total)
...
if __name__ == '__main__':
    ...
    for i in range(len(user_list)):
        pool.apply_async(get_all_tweets,([user_list[i],i%len(api_list)],))
    pool.close()
    pool.join()
