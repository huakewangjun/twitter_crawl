...
def save_tweets(keyword,new_tweets):
    for i in range(0,len(new_tweets)):
        ...
        if (u'\u2026' in content) or (in_reply_to_status_id is not None) or (retweeted_reply_to_status_id is not None):
            while True:
                try:
...
def get_tweets_by_keyword(keyword):
    ...
    if not results[0]:
        while True:
            try:
                new_tweets=api.search(q=keyword,count=20,result_type='recent')
                break
            except tweepy.TweepError, e:
                print 'TweepError when get search(q='+keyword+',count=20):\n',e
                time.sleep(10)
        while (len(new_tweets) > 0):
            ...
            while True:
                try:
                    new_tweets = api.search(q=keyword,count=20, max_id=oldest,result_type='recent')
                    break
                except tweepy.TweepError, e:
                    print 'TweepError when get search(q='+keyword+',count=20,max_id='+str(oldest)+'):\n',e
                    time.sleep(10)
    else:
        newest=results[0]
        oldest=results[1]-1
        ...
        while True:
            while True:
                try:
                    new_tweets = api.search(q=keyword,count=20, since_id=newest,result_type='recent')
                    break
                except tweepy.TweepError, e:
                    print 'TweepError when get search(q='+keyword+',count=20,since_id='+str(newest)+'):\n',e
                    time.sleep(10)
            if new_tweets:
                save_tweets(keyword,new_tweets)
